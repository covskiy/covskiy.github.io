# NewLayout — новый layout-движок (с нуля)

Замена старого `src/components/layout/` на `src/components/NewLayout/`,
...
├── engine.ts                # createNewLayoutEngine (внешний движок)
├── gsap/
...
│   └── NewLayoutProvider.tsx # реакция на bp/route/resize, владеет движком
├── slots/
...
├── NewAppRoot.tsx           # композиция: NewLayoutProvider → GsapProvider →
│                            #   GsapLayoutBridge → LayoutRoot → слоты
└── engine.ts
```
...
| #   | Решение                                                                                                                                                                              |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1  | Чистый с нуля. `src/components/NewLayout/` строится пустым; существующий `LayoutState.ts` удаляется. Переноса кода из старого движка нет.                                          |
...
| D8  | GSAP-слой в `NewLayout/gsap/`: `gsapBus.ts` (60fps-шина), `GsapProvider.tsx` (инфраструктура), `GsapLayoutBridge.tsx` (адаптер 60fps → дискретные события).                         |
...
GSAP 60fps events (scroll:frame, frame)
  ...
  → NewLayoutEngine (state machine + LayoutSnapshot)
...
| `NewLayoutProvider`| React-обёртка движка; реакция на `useBreakpoint`/`useLocation`/resize.                          |
...
import {
  ...
  useLayoutSnapshot,
  useLayoutSend,
} from 'src/components/NewLayout/context/layoutContexts';
import { useRegisterHomeSpacer } from 'src/components/NewLayout/gsap/GsapLayoutBridge';
